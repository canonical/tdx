...
class TdxCoexist(unittest.TestCase):
    """
    """
    def setUp(self):
        self.qm = Qemu.QemuMachine('td',
                                   QemuEfiMachine.OVMF_Q35_TDX,
                                   service_blacklist = [QemuMachineService.QEMU_MACHINE_PORT_FWD])
        self.qm.run()
        self.qm_normal = Qemu.QemuMachine('normal',
    ...
    def test_coexist_boot(self):
        """
        Boot check
        """
        m = Qemu.QemuMonitor(self.qm)
        m.wait_for_state('running')
        m_normal = Qemu.QemuMonitor(self.qm_normal)
