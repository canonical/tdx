...
class QemuSSH():
    ...
    def __init__(self,
        ...
        assert qemu.fwd_port != None

        # prevent paramiko to do spurious logs on stdout
        paramiko.util.log_to_file(filename=f'{qemu.workdir_name}/paramiko-log.txt', level=logging.DEBUG)
        self._wait_and_connect(qemu.fwd_port, timeout=timeout)
...
class QemuMachine:
    ...
    def _create_image(self):
        ...
        orig_image_path=f'{script_path}/../data/tdx-guest.qcow2'
        self.image_path=f'{self.workdir_name}/image.qcow2'
        subprocess.check_call(f'qemu-img create -f qcow2 -b {orig_image_path} -F qcow2 {self.image_path} && sync', shell=True)
